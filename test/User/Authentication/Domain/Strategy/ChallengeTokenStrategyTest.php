...
use Webify\User\Authentication\Domain\Strategy\ChallengeTokenStrategy;
use Webify\User\Authentication\Domain\ValueObject\UserId;

/**
 * Tests for the ChallengeTokenStrategy.
 *
 * @internal
 */
#[AllowMockObjectsWithoutExpectations]
#[CoversClass(ChallengeTokenStrategy::class)]
#[CoversMethod(ChallengeTokenStrategy::class, 'getIdentifier')]
#[CoversMethod(ChallengeTokenStrategy::class, 'isSupported')]
#[CoversMethod(ChallengeTokenStrategy::class, 'initiate')]
final class ChallengeTokenStrategyTest extends TestCase
{
	/**
	 ...
	 */
	private ChallengeTokenStrategy $strategy;
	...
	protected function setUp(): void
	{
		...
		$this->strategy            = new ChallengeTokenStrategy($issueChallenge, $this->delivery);
	}
	...
	{
		$this->assertSame('challenge_token', $this->strategy->getIdentifier());
	}
	...
	/**
	 * Test that initiate issues a challenge token and delivers it.
	 */
	#[Test]
	public function testInitiateIssuesAndDeliversToken(): void
	{
		...
		$this->delivery
			->expects($this->once())
			->method('deliver')
			->with($userId, 'token', $this->matchesRegularExpression('/^[0-9a-f]{64}$/'))
		;
		$this->eventPublisher
			->expects($this->once())
			->method('publish')
